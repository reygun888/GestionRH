...
//----------HEURES SUPP.-----------
console.log('Début de la manipulation des données d\'heures supplémentaires');

console.log('heuresSupData :', heuresSupData);
...
heuresSupData.forEach((heuresSup, i) => { // Ajouter l'index i ici
    ...
    console.log(`Clé générée pour la date ${heuresSup.date} : ${key}`);
    joursHeuresSup[key] = joursHeuresSup[key] || [];
    console.log(`Données stockées dans joursHeuresSup pour la clé ${key} :`, joursHeuresSup[key]);
    joursHeuresSup[key].push(heuresSup.nbHeures);
    console.log(`Données stockées dans joursHeuresSup pour la clé ${key} après l'ajout :`, joursHeuresSup[key]);
   
});
console.log('joursHeuresSup :', joursHeuresSup);
    ...
    if (joursHeuresSup.hasOwnProperty(key)) {
        joursHeuresSup[key].forEach((heuresSup, i) => {
            if (typeof heuresSup === 'string') {
                ...
                console.log(`Ajout de ${heures}:${minutes} d'heures supplémentaires à nombreHeuresSup (total avant l'ajout : ${nombreHeuresSup})`);
                nombreHeuresSup += heures + minutes / 60;
                console.log(`Ajout de ${heures}:${minutes} d'heures supplémentaires à nombreHeuresSup (total après l'ajout : ${nombreHeuresSup})`);
            } else {
                console.log(`Ajout de ${heuresSup} d'heures supplémentaires à nombreHeuresSup (total avant l'ajout : ${nombreHeuresSup})`);
                nombreHeuresSup += heuresSup;
                console.log(`Ajout de ${heuresSup} d'heures supplémentaires à nombreHeuresSup (total après l'ajout : ${nombreHeuresSup})`);
            }
        });
    }
}
...
console.log(`Nombre total d'heures supplémentaires : ${heuresSupFormatees}`);
// Remplir l'objet avec le jour d'heures supp.
// heuresSupData.forEach(heuresSup => {
//     const dateDebut = new Date(heuresSup.date);
//     const dateFin = new Date(heuresSup.date);

//     // Boucler sur chaque jour d'heures Supp. et stocker l'information
//     for (let currentDate = dateDebut; currentDate <= dateFin; currentDate.setDate(currentDate.getDate() + 1)) {
//         const key = `${currentDate.getFullYear()}-${(currentDate.getMonth() + 1).toString().padStart(2, '0')}-${currentDate.getDate().toString().padStart(2, '0')}`;
//         joursHeuresSup[key] = true;
//     }
// });
heuresSupData.forEach((heuresSup, i) => { // Ajouter l'index i ici
    const dateDebut = new Date(heuresSup.date);
    ...
    if (heuresSup.duree) {
        ...
        console.log(`Ajout de ${dureeHeuresSup}ms d'heures supplémentaires pour la date ${key}`);
    }
});
    ...
    if (estJourHeuresSup) {
        ...
        dureesHeuresSup.forEach(duree => {
            if (typeof duree === 'string') { // utiliser "duree" à la place de "heuresSup"
                ...
                console.log(`Ajout de ${duree} d'heures supplémentaires à nombreHeuresSup (total avant l'ajout : ${nombreHeuresSup})`);
                nombreHeuresSup += duree;
                console.log(`Ajout de ${duree} d'heures supplémentaires à nombreHeuresSup (total après l'ajout : ${nombreHeuresSup})`);
            }
        });
    }
...
// Mettre à jour le nombre d'absences dans le HTML
document.getElementById('nombre-absences').textContent = nombreAbsences;

// Mettre à jour le nombre de congés dans le HTML
document.getElementById('nombre-conges').textContent = nombreConges;

// Formater et mettre à jour le nombre d'heures supplémentaires dans le HTML
// const formattedHeuresSup = formatHeuresSup(nombreHeuresSup);
document.getElementById('nombre-heuresSup').textContent = heuresSupFormatees;
// document.getElementById('nombre-heuresSup').textContent = nombreHeuresSup; // ou .textContent = formatHeuresSup
...
function afficherEvenementsDuJour(absences, conges, heuresSup) {
    ...
    if (absences.length === 0 && conges.length === 0 && heuresSup.length === 0) {
        ...
        console.log("Aucun évènement pour ce jour.");
        infoContainer.innerHTML = '<h3 class="text-center m-5">Aucun évènement</h3>';
    } else {
        // Générer le contenu HTML pour les absences et les congés du jour
        console.log("Affichage des événements pour ce jour :");
    ...
    if (heuresSup.length > 0) {
        ...
        //Customiser l'affichage de la date dans le tableau
...
// // Fonction pour formater les heures supplémentaires
// function formatHeuresSup(nombreHeuresSup) {
//     // Conversion des heures supplémentaires en heures et minutes
//     const heures = Math.floor(nombreHeuresSup / 60);
//     const minutes = nombreHeuresSup % 60;

//     // Formater le résultat
//     const formattedHeuresSup = `${heures}h ${minutes}min`;

//     return formattedHeuresSup;
// }

// function mettreAJourCompteursFiltres(absences, conges, heuresSup, employeeName, mois, annee) {
//     // Filtrer les événements en fonction du mois et de l'année actuels (si nécessaire)
...
//         const dateConge = new Date(conge.date);
//         return dateConge.getMonth() === mois && dateConge.getFullYear() === annee;
//     });
//     const filteredHeuresSup = heuresSup.filter(heuresSup => {
//         const dateHeuresSup = new Date(heuresSup.date);
//         return dateHeuresSup.getMonth() === mois && dateHeuresSup.getFullYear() === annee;
//     });
...
//     // Formater et mettre à jour le nombre d'heures supplémentaires dans le HTML
//     const formattedHeuresSup = formatHeuresSup(nombreHeuresSup);
//     document.getElementById('nombre-heuresSup').textContent = formattedHeuresSup;
